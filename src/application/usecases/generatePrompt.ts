...
12. design は任意。ただしデザイン希望がある場合は design を含め、layout.preset は timeline/ticket/metro/cards/serpentine のいずれかにする。
13. design.layout.cornerRadius は 0〜28 の範囲にする。density は compact/comfortable のいずれかにする。
14. design.palette は #RGB または #RRGGBB のみ（例: "#d9ab23"）。読みやすさ（コントラスト）を最優先にする。
15. JSONキーと文字列値を囲む引用符は半角のダブルクォート (") のみを使用する。
16. 全角・スマートクォート（” “ ’）は使用禁止。これらの文字を一切出力しない。
17. 文字列値の中に " を含める場合は必ず \\" へエスケープし、必要に応じてバックスラッシュは \\\\ とする。
18. 出力前に自己検証し、未エスケープの " が1つでも残らないように最終チェックする。
