...
3. Mistake: Failing to validate `id` or `d` query params before decrypt flow.
   Next-time fix: Validate early, show clear recovery path, never attempt undefined decrypt input.
...
2. Store only passhash metadata in localStorage (`shiori:passhash:<id>`), never raw itinerary plaintext.
