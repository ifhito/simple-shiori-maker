...
- 新規暗号フォーマットを `v6(compact + brotli + binary-in-KV)` へ更新（`v3/v4/v5` は後方互換復号）: 実装済み
...
- 暗号: AES-256-GCM
- 新規共有データ: `d = [0x06 | salt(16) | iv(12) | ciphertext]`（KVにバイナリ保存）
- 圧縮: 新規発行は brotli（暗号化前に圧縮）。後方互換で `v4(brotli)` 読み取りも維持
