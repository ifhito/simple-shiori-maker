...
追加:
- `links:{SHA-256(合言葉)}` -> 暗号化リンク一覧JSON（TTL: 30日、アクセス時リセット）
...
`sessionStorage` で `passphraseHash` のみをタブ閉鎖まで一時保持（合言葉の平文は保存しない）
`cachePassphraseHash()`, `getCachedPassphraseHash()`, `clearCachedPassphraseHash()`
...
`POST /api/links/load`
データなしは `{ links: [] }` で `200`
note: KVキーが `SHA-256(合言葉)` 由来のため、合言葉が違う場合は別キーになり「0件」として扱われる（不一致を確定できない）
レート制限適用
...
`sessionStorage` に `passphraseHash` キャッシュがあれば自動ロード
...
既存リンク一覧を `load` -> 追記 -> `save` でKV保存 -> `sessionStorage` に `passphraseHash` をキャッシュ
...
`LINKS_TTL_SECONDS = "2592000"`（30日）
`MAX_LINKS_COUNT = "200"`
`MAX_LINKS_PLAINTEXT_BYTES = "32768"`
...
現状方針: `sessionStorage` に保存するのは `SHA-256(passphrase)`（`passphraseHash`）のみ（合言葉の平文は保存しない）。
よりセキュリティ重視なら「合言葉は保存せず都度入力」へ寄せる。
...
- 暗号化: リンク一覧は `passphraseHash` を秘密鍵として `AES-256-GCM` 暗号化し、`Uint8Array` でKVに直接格納
- 合言葉キャッシュ: `sessionStorage` に `passphraseHash` のみを保存（タブ閉鎖で自動消去）
